...
void InitializeLattice(int, mat &, double&, double&, std::uniform_real_distribution<double>, std::mt19937_64);
...
void MetropolisSampling(int NSpins, int MCcycles, double Temperature, vec &ExpectationValues)
{
  ...
  ExpectationValues(0) = 0;    ExpectationValues(1) =0;
  ExpectationValues(2) =0;
  ExpectationValues(3) =0;
  ExpectationValues(4) =0;
  // initialize array for expectation values
  InitializeLattice(NSpins, SpinMatrix, Energy, MagneticMoment, RandomNumberGenerator, gen);
...
void InitializeLattice(int NSpins, mat &SpinMatrix,  double& Energy, double& MagneticMoment, std::uniform_real_distribution<double> RandomNumberGen, std::mt19937_64 gen)
{
  // setup spin matrix and initial magnetization
  for(int x =0; x < NSpins; x++) {
    for (int y= 0; y < NSpins; y++){
      //if(RandomNumberGen(gen)<0.5){
         // SpinMatrix(x,y)=-1;
      //}else{
...
void WriteResultstoFile(int NSpins, int MCcycles, double temperature, vec ExpectationValues)
{
  ...
  ofile << setw(15) << setprecision(8) << Evariance/NSpins/NSpins;
  ofile << setw(15) << setprecision(8) << M_ExpectationValues/NSpins/NSpins;
  ofile << setw(15) << setprecision(8) << Mvariance/NSpins/NSpins;
